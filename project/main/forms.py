...
class TransactionFormMixin(forms.ModelForm):
    ...
    def __init__(self, user, *args, **kwargs):
        ...
        if self.instance.pk:
            ...
            """
            self.fields['category'].queryset = Category.objects.filter(type=self.instance.subcategory.category.type)
        ...
        if self.data:
            ...
            if self.data.get("type"):
                self.fields['category'].queryset = Category.objects.filter(type=int(self.data.get("type")))
                self.set_selects(category_enable=True)
            else:
                self.set_selects(False, False)
            
            if self.data.get("category"):
                self.fields["subcategory"].queryset = Subcategory.objects.filter(category=int(self.data.get("category")))
                self.set_selects(category_enable=True, subcategory_enable=True)
            else:
